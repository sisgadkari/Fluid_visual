...
with tab1:
    ...
    with col2:
        ...
        def generate_animated_pitot_plot(h_inst, velocity, rho_fluid, rho_mano):
            """Generate animated Pitot-static tube visualization with flowing particles"""
            ...
            # Generate particle starting positions (distributed across the inlet)
            np.random.seed(42)  # For reproducibility
            particle_y_starts = np.linspace(-field_height/2 * 0.85, field_height/2 * 0.85, n_particles)
            
            # Calculate particle paths - particles flow from left, some hit stagnation point
            def calculate_particle_position(y_start, progress, vel):
                """Calculate particle position based on starting y and progress (0-1)"""
                # Base x position (moves right with flow)
                x_base = -2 + progress * (field_width + 2) * (vel / 50 + 0.3)  # Speed scales with velocity
                
                # Check if particle would hit the probe
                if abs(y_start) < probe_outer_r * 1.2:
                    # This particle approaches stagnation point
                    if x_base > -0.5:
                        # Slow down as approaching stagnation
                        slowdown = np.exp(-((x_base + 0.5) * 3))
                        x_pos = -0.5 + (x_base + 0.5) * slowdown * 0.3
                        x_pos = min(x_pos, -0.05)  # Stop just before nose
                        
                        # Particle drifts slightly toward centerline (stagnation streamline)
                        y_pos = y_start * (1 - (x_pos + 2) / 1.5) ** 0.5 if x_pos > -1.5 else y_start
                        y_pos = y_pos * max(0.1, 1 - progress * 0.5)  # Compress toward center
                    else:
                        x_pos = x_base
                        y_pos = y_start
                else:
                    # Particle flows around the probe
                    x_pos = x_base
                    
                    # Deflection around probe
                    if -0.5 < x_pos < probe_length:
                        deflection = np.exp(-((x_pos - 1) ** 2) / 3) * 0.3
                        if abs(y_start) < probe_outer_r * 2:
                            y_pos = y_start + np.sign(y_start) * deflection * (probe_outer_r * 2 - abs(y_start))
                        else:
                            y_pos = y_start + np.sign(y_start) * deflection * 0.1
                    else:
                        y_pos = y_start
                
                # Wrap around when particle exits right side
            ...
            for frame_idx in range(n_frames):
                ...
                # Calculate particle positions for this frame
                particle_x = []
                particle_y = []
                particle_colors = []
                particle_sizes = []
                
                for i, y_start in enumerate(particle_y_starts):
                    # Each particle has a phase offset for continuous flow appearance
                    phase = (frame_idx / n_frames + i / n_particles) % 1.0
                    
                    x_pos, y_pos = calculate_particle_position(y_start, phase, velocity)
                    
                    # Skip particles that are outside visible area
                    if -2.5 < x_pos < field_width + 0.5:
                        particle_x.append(x_pos)
                        particle_y.append(y_pos)
                        
                        # Color based on proximity to stagnation point
                        dist_to_stag = np.sqrt(x_pos**2 + y_pos**2)
                        if dist_to_stag < 0.5 and abs(y_start) < probe_outer_r * 1.2:
                            # Near stagnation - turn red/orange
                            particle_colors.append(f'rgba(255, {int(100 + 155 * dist_to_stag)}, 0, 0.9)')
                            particle_sizes.append(10 + (0.5 - dist_to_stag) * 15)  # Grow near stagnation
                        else:
                            # Normal flow - blue
                            particle_colors.append('rgba(30, 100, 200, 0.8)')
                            particle_sizes.append(8)
                
                # Create scatter trace for particles
                ...
                # Add stagnation point glow effect (pulsing)
                glow_intensity = 0.3 + 0.2 * np.sin(frame_idx / n_frames * 2 * np.pi)
                if velocity > 0:
                    # Stagnation point glow
                    for glow_r in [0.15, 0.25, 0.35]:
                        frame_data.append(go.Scatter(
                            x=[0],
                            y=[0],
                            mode='markers',
                            marker=dict(
                                size=glow_r * 100,
                                color=f'rgba(255, {int(150 - glow_r * 100)}, 0, {glow_intensity * (0.5 - glow_r)})',
                            ),
                            hoverinfo='none',
                            showlegend=False
                        ))
            ...
            # Create the base figure with static elements
            fig = generate_pitot_plot(h_inst, velocity)
            
            # Add initial particle positions (frame 0)
            initial_particle_x = []
            initial_particle_y = []
            initial_colors = []
            initial_sizes = []
            ...
            for i, y_start in enumerate(particle_y_starts):
                ...
                if -2.5 < x_pos < field_width + 0.5:
                    initial_particle_x.append(x_pos)
                    initial_particle_y.append(y_pos)
                    
                    dist_to_stag = np.sqrt(x_pos**2 + y_pos**2)
                    if dist_to_stag < 0.5 and abs(y_start) < probe_outer_r * 1.2:
                        initial_colors.append(f'rgba(255, {int(100 + 155 * dist_to_stag)}, 0, 0.9)')
                        initial_sizes.append(10 + (0.5 - dist_to_stag) * 15)
                    else:
                        initial_colors.append('rgba(30, 100, 200, 0.8)')
                        initial_sizes.append(8)
            
            fig.add_trace(go.Scatter(
                x=initial_particle_x,
                y=initial_particle_y,
                mode='markers',
                marker=dict(
                    size=initial_sizes,
            ...
            if velocity > 0:
                for glow_r in [0.15, 0.25, 0.35]:
                    fig.add_trace(go.Scatter(
                        ...
                        marker=dict(
                            size=glow_r * 100,
                            color=f'rgba(255, {int(150 - glow_r * 100)}, 0, {0.3 * (0.5 - glow_r)})',
                        ),
                        hoverinfo='none',
                        showlegend=False,
                        name=f'glow_{glow_r}'
            ...
            # Add velocity gauge in top right corner
            gauge_x = field_width - 1.5
            gauge_y = 2.0
            gauge_radius = 0.8
            
            # Gauge background
            theta_gauge = np.linspace(np.pi, 0, 50)
            x_gauge_bg = gauge_x + gauge_radius * np.cos(theta_gauge)
            y_gauge_bg = gauge_y + gauge_radius * np.sin(theta_gauge)
            
            fig.add_trace(go.Scatter(
                x=np.concatenate([[gauge_x - gauge_radius], x_gauge_bg, [gauge_x + gauge_radius]]),
                y=np.concatenate([[gauge_y], y_gauge_bg, [gauge_y]]),
                fill='toself',
            ...
            # Gauge colored sectors (green-yellow-red)
            sectors = [
                (np.pi, np.pi * 2/3, 'rgba(34, 197, 94, 0.7)'),      # Green (low speed)
                (np.pi * 2/3, np.pi * 1/3, 'rgba(234, 179, 8, 0.7)'),  # Yellow (medium)
                (np.pi * 1/3, 0, 'rgba(239, 68, 68, 0.7)')            # Red (high speed)
            ]
            
            for start_angle, end_angle, color in sectors:
                theta_sector = np.linspace(start_angle, end_angle, 20)
                x_sector = np.concatenate([[gauge_x], gauge_x + gauge_radius * 0.85 * np.cos(theta_sector), [gauge_x]])
                y_sector = np.concatenate([[gauge_y], gauge_y + gauge_radius * 0.85 * np.sin(theta_sector), [gauge_y]])
            ...
            max_velocity_gauge = 100  # m/s for full scale
            tick_velocities = [0, 25, 50, 75, 100]
            
            for tick_vel in tick_velocities:
                tick_angle = np.pi * (1 - tick_vel / max_velocity_gauge)
                tick_x_inner = gauge_x + gauge_radius * 0.7 * np.cos(tick_angle)
                tick_y_inner = gauge_y + gauge_radius * 0.7 * np.sin(tick_angle)
                tick_x_outer = gauge_x + gauge_radius * 0.85 * np.cos(tick_angle)
                tick_y_outer = gauge_y + gauge_radius * 0.85 * np.sin(tick_angle)
                
                fig.add_shape(type="line",
                             x0=tick_x_inner, y0=tick_y_inner,
                             x1=tick_x_outer, y1=tick_y_outer,
                             line=dict(color="black", width=2))
                
                # Label
                label_x = gauge_x + gauge_radius * 0.55 * np.cos(tick_angle)
                label_y = gauge_y + gauge_radius * 0.55 * np.sin(tick_angle)
                fig.add_annotation(x=label_x, y=label_y, text=str(tick_vel),
                                  showarrow=False, font=dict(size=8))
            
            # Gauge needle
            needle_angle = np.pi * (1 - min(velocity, max_velocity_gauge) / max_velocity_gauge)
            needle_length = gauge_radius * 0.75
            needle_x = gauge_x + needle_length * np.cos(needle_angle)
            needle_y = gauge_y + needle_length * np.sin(needle_angle)
            ...
            # Gauge title
            fig.add_annotation(x=gauge_x, y=gauge_y - 0.15,
                              text="m/s", showarrow=False, font=dict(size=10, color='black'))
            fig.add_annotation(x=gauge_x, y=gauge_y + gauge_radius + 0.15,
                              text="<b>Velocity</b>", showarrow=False, font=dict(size=11, color='black'))
        ...
        if U > 0:
            st.markdown("""
            <div style='background: linear-gradient(90deg, rgba(30,100,200,0.8) 0%, rgba(255,200,0,0.9) 50%, rgba(255,100,0,0.9) 100%); 
                        height: 15px; border-radius: 5px; margin: 10px 0;'></div>
            <div style='display: flex; justify-content: space-between; font-size: 12px; color: #666;'>
                <span>Normal Flow</span>
                <span>Approaching Stagnation</span>
                <span>At Stagnation Point</span>
            </div>
            """, unsafe_allow_html=True)
            st.caption("💡 **Tip**: Click '▶ Play' to animate the flow particles. Particles slow down and change color as they approach the stagnation point at the probe nose.")
