...
tab1, tab2, tab3, tab4 = st.tabs(["🎯 Interactive Simulation", "📚 Understanding Capillary Action", "📋 Real-World Applications", "🎓 Test Your Understanding"])
...
with tab4:
    ...
    st.markdown("""
    Ready to test your knowledge? These questions go beyond memorization - they challenge you to apply 
    capillary action concepts to real-world scenarios. Think carefully about each situation!
    """)
    ...
    # Question 1
    st.markdown("### Question 1: The Astronaut's Coffee Predicament ☕🚀")
    st.markdown("""
    An astronaut aboard the International Space Station accidentally spills coffee near a narrow gap 
    between two metal plates (2 mm apart). The coffee is attracted to the gap. What happens next?
    """)
    
    q1_options = [
        "The coffee floats away randomly in all directions",
        "The coffee gets pulled into the gap and spreads along it via capillary action",
        "The coffee cannot enter the gap because there's no gravity",
        "The coffee forms perfect spherical droplets and stays in place"
    ]
    
    q1 = st.radio("Select your answer:", q1_options, key="q1", index=None)
    ...
    # Question 2
    st.markdown("### Question 2: The Desert Survival Challenge 🌵💧")
    st.markdown("""
    You're stranded in a desert with a bottle of water and several empty tubes of different diameters 
    (0.5 mm, 2 mm, and 5 mm). You need to design a simple device to slowly wick water from your bottle 
    to keep a piece of cloth damp (for cooling). Which setup would work BEST?
    """)
    
    q2_options = [
        "Use the 5 mm tube - larger diameter means faster water delivery",
        "Use the 0.5 mm tube - smallest diameter gives highest capillary rise and steady wicking",
        "Use the 2 mm tube - it's the middle option so it's the most balanced",
        "Diameter doesn't matter - capillary action is the same regardless of tube size"
    ]
    
    q2 = st.radio("Select your answer:", q2_options, key="q2", index=None)
    ...
    # Question 3
    st.markdown("### Question 3: The Tea vs Coffee Mystery 🍵☕")
    st.markdown("""
    You accidentally spill both tea and coffee on your lab bench. You notice that when these liquids 
    touch a paper towel, the tea spreads and climbs faster than the coffee. Your colleague suggests 
    this is because tea has lower surface tension than coffee. Is this explanation correct?
    
    **Additional info**: Paper towel fibers create very small capillary channels (≈ 0.1 mm diameter).
    """)
    
    q3_options = [
        "Yes - lower surface tension always means faster capillary rise",
        "No - the rate depends on viscosity, not surface tension. Tea is probably less viscous.",
        "No - this is wrong. Higher surface tension actually causes faster rise.",
        "Yes, but only because tea also has lower density than coffee"
    ]
    
    q3 = st.radio("Select your answer:", q3_options, key="q3", index=None)
    ...
    if st.session_state.quiz_submitted and st.session_state.show_explanations:
        st.markdown("---")
        st.markdown("## 📊 Your Results")
        
        # Correct answers
        correct_answers = {
            'q1': "The coffee gets pulled into the gap and spreads along it via capillary action",
            'q2': "Use the 0.5 mm tube - smallest diameter gives highest capillary rise and steady wicking",
            'q3': "No - the rate depends on viscosity, not surface tension. Tea is probably less viscous."
        }
        
        # Calculate score
        score = 0
        for q_key, correct_ans in correct_answers.items():
            if st.session_state.quiz_answers.get(q_key) == correct_ans:
                score += 1
        ...
        if score == 3:
            ...
            st.balloons()
            st.markdown("**Outstanding!** You have an excellent understanding of capillary action principles and their real-world applications!")
        elif score == 2:
            st.info(f"👍 **Good Job: {score}/3 ({score_percentage:.0f}%)**")
            st.markdown("**Well done!** You grasp the key concepts. Review the explanations below to master the remaining details.")
        else:
            st.warning(f"📚 **Score: {score}/3 ({score_percentage:.0f}%)**")
            st.markdown("**Keep learning!** Review the explanations carefully and revisit the simulation tab to strengthen your understanding.")
        
        st.markdown("---")
        st.markdown("## 💡 Detailed Explanations")
        
        # Question 1 Explanation
        st.markdown("### Question 1: The Astronaut's Coffee")
        user_q1 = st.session_state.quiz_answers.get('q1')
        ...
        **Explanation:**
        ...
        Capillary action does NOT require gravity! It's driven by surface tension forces and molecular adhesion 
        between the liquid and solid surfaces. The pressure difference across the curved meniscus 
        (Young-Laplace equation: ΔP = 2σ/r) is what pulls liquid into narrow gaps.
        
        **Key insight:** In zero gravity:
        - ✅ Surface tension still works (molecular forces)
        - ✅ Capillary action still occurs
        - ❌ Gravitational hydrostatic pressure is absent
        - **Result:** Capillary action is actually STRONGER in space because there's no gravitational force opposing it!
        
        **Real application:** NASA designs special coffee cups for space that use capillary channels to 
        guide liquid to the astronaut's mouth, replacing the normal "pouring" that requires gravity.
        """)
        ...
        st.markdown("### Question 2: The Desert Survival Challenge")
        user_q2 = st.session_state.quiz_answers.get('q2')
        ...
        **Explanation:**
        ...
        The capillary rise formula shows an **inverse relationship** with diameter:
        """)
        st.latex(r"h = \frac{4\sigma\cos\theta}{\rho g d}")
        
        st.markdown("""
        **Calculations for water:**
        - 0.5 mm tube: h ≈ 30 mm (3 cm rise!)
        - 2 mm tube: h ≈ 7.5 mm 
        - 5 mm tube: h ≈ 3 mm
        
        **Why smaller is better for wicking:**
        1. **Higher capillary rise** (h ∝ 1/d): The 0.5 mm tube pulls water 10× higher than the 5 mm tube
        2. **Steady flow rate**: Small diameter provides consistent, slow wicking
        3. **Better distribution**: The water spreads evenly through the cloth's small pores
        4. **Less waste**: Controlled delivery prevents rapid evaporation
        
        **Real application:** This principle is used in:
        - Wicking fabrics for sportswear (microfibers)
        - Medical wound dressings (absorb fluid slowly)
        - Oil lamp wicks (steady fuel delivery)
        ...
        st.markdown("### Question 3: The Tea vs Coffee Mystery")
        user_q3 = st.session_state.quiz_answers.get('q3')
        ...
        **Explanation:**
        ...
        This is a trick question that tests deeper understanding! While surface tension affects the 
        **maximum height** (h ∝ σ), the **rate of rise** depends primarily on **viscosity**.
        
        **Two different concepts:**
        
        1. **Maximum capillary height** (equilibrium):
        """)
        st.latex(r"h_{max} = \frac{4\sigma\cos\theta}{\rho g d}")
        st.markdown("   - Higher σ → Higher h_max ✓")
        
        st.markdown("""
        2. **Rate of capillary rise** (dynamics):
        """)
        st.latex(r"\frac{dh}{dt} \propto \frac{\sigma}{\mu}")
        st.markdown("""
        - Rate increases with σ (driving force)
        - Rate **decreases** with μ (viscosity = resistance)
        - The ratio σ/μ determines speed!
        
        **Why tea spreads faster:**
        - Tea: σ ≈ 0.065 N/m, μ ≈ 0.001 Pa·s → σ/μ = 65
        - Coffee: σ ≈ 0.068 N/m, μ ≈ 0.0015 Pa·s → σ/μ = 45
        - Tea's **lower viscosity** makes it flow faster despite similar surface tension
        
        **Real-world analogy:** Think of honey (high viscosity) vs water (low viscosity) climbing a paper 
        towel. Water climbs much faster even though honey might have similar surface tension, because 
        honey's thick, sticky nature (high μ) resists flow.
        
        **Applications:**
        - **Inkjet printing**: Low-viscosity inks spread faster in paper fibers
        - **Chromatography**: Solvent viscosity affects separation speed
        - **Textile absorption**: Fast-drying fabrics need low-viscosity fluids
        - **Biological wicking**: Plant xylem optimizes sap viscosity for efficient water transport
        """)
        ...
        if score < 3:
            ...
            **Revisit these sections:**
            ...
            if not is_correct_q1:
                st.markdown("- ⭐ **Interactive Simulation**: Experiment with different tube diameters to see h ∝ 1/d relationship")
                st.markdown("- ⭐ **Understanding Tab**: Review the Young-Laplace equation and pressure differences")
            
            if not is_correct_q2:
                st.markdown("- ⭐ **Interactive Simulation**: Compare heights for 0.5 mm vs 5 mm tubes")
                st.markdown("- ⭐ **Real-World Applications**: Read about wicking in textiles and medical devices")
            
            if not is_correct_q3:
                st.markdown("- ⭐ **Understanding Tab**: Study the difference between equilibrium height and rise rate")
                st.markdown("- ⭐ **Real-World Applications**: Learn about chromatography and inkjet printing")
        else:
            st.success("""
            **Excellent work!** You've mastered capillary action concepts. 
            
            **Challenge yourself further:**
            - Try deriving the capillary rise equation from first principles
            - Research advanced topics like capillary pressure in porous media
            - Explore contact angle hysteresis in real materials
            - Investigate the Washburn equation for dynamic capillary flow
            """)
        
        st.markdown("---")
        st.info("💡 **Pro tip**: Use the Interactive Simulation tab to test the concepts discussed in these questions!")
