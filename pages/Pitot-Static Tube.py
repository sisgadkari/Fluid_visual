...
with col2:
    ...
    def generate_pitot_plot(h_inst, velocity):
        ...
        if show_details:
            ...
            fig.add_shape(type="circle", x0=-0.02, y0=-probe_inner_r/2,
                         x1=0.02, y1=probe_inner_r/2,
            ...
            for i in range(n_static_ports):
                ...
                port_y = probe_outer_r * 0.8 * np.sin(angle)
                if abs(port_y) > probe_inner_r:
                    fig.add_shape(type="circle",
                                 x0=static_port_x-0.02, y0=port_y-0.02,
                                 x1=static_port_x+0.02, y1=port_y+0.02,
                                 fillcolor=static_color, line_width=0)
        ...
        fig.add_shape(type="line", 
                     x0=left_tube_x + (tube_outer_radius - tube_inner_radius) - 0.01, 
                     y0=level_left, 
                     x1=left_tube_x + (tube_outer_radius - tube_inner_radius) + 0.01, 
                     y1=level_left, 
                     line=dict(color="black", width=2))
        fig.add_shape(type="line", 
                     x0=right_tube_x - (tube_outer_radius - tube_inner_radius) - 0.01, 
                     y0=level_right, 
                     x1=right_tube_x - (tube_outer_radius - tube_inner_radius) + 0.01, 
                     y1=level_right, 
                     line=dict(color="black", width=2))
        ...
        if velocity > 0:
            ...
            fig.add_annotation(x=probe_length/2, y=-probe_outer_r-0.5,
        ...
        # Title
        fig.add_annotation(x=probe_length/2, y=probe_outer_r+0.6,
